 ...
 *      25.08.2020  HL  ISR_error(void), ISR_default(void) and INTERRUPT VECTOR TABLE added
 ...
 *** Description **************************************************************
 *
 * Timer6 sets the time ticks for the WS2812B rainbow fader.
...
void TIM6_DAC_IRQHandler(void) {
	...
	while ((ADC1->ISR & ADC_ISR_EOC) == 0)
		// wait till finished
		;
	ADC_value = (uint16_t) (ADC1->DR);		// read ADC value
	ADC1->ISR &= ~ADC_ISR_EOC;				// reset the end of conversion flag

	// normalize result to use in PWM
	uint32_t LED_value = (ADC_value * 100) / 4096;// 12bit ADC value auf 100 normieren und ins CCR abspeichern->setzt LED duty cycle
	TIM3->CCR1 = LED_value;
...
void TIM3_IRQHandler(void) {
	TIM3->SR &= ~ TIM_SR_UIF;		// clear INT flag
}
...
void (*const paIsrFunc[98])(void) =
{
	...
	TIM3_IRQHandler, /* 29 (0x000000B4) (prio: 36) TIM3 interrupt */
