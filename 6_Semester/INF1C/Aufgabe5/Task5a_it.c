...
/* ------------------------------------------------------------------------------------ *\
 * method:  void CAN1_RX0_IRQHandler(void)
 *
 * CAN frame arrived.
...
void EXTI0_IRQHandler(void)
{
    ...
    if (!(GPIOB->IDR & MASK_S0))        // button S0 pressed
    {
        GPIOC->ODR ^= (1 << 4);         //LED0 toggle
        tx_candata[0] = LED_red_efiCAN;
        CAN_send8(tx_candata);
    }

    EXTI->PR1 |= EXTI_PR1_PIF0;                    // reset pending register
}
...
void EXTI1_IRQHandler(void)
{
    ...
    if (!(GPIOB->IDR & MASK_S1))        // button S1 pressed
    {
        GPIOC->ODR ^= (1 << 5);         //LED1 toggle
        tx_candata[0] = LED_yellow_efiCAN;
        CAN_send8(tx_candata);
    }

    EXTI->PR1 |= EXTI_PR1_PIF1;                    // reset pending register
}
...
void EXTI2_IRQHandler(void)
{
    ...
    if (!(GPIOB->IDR & MASK_S2))        // button S2 pressed
    {
        GPIOC->ODR ^= (1 << 6);         //LED2 toggle
        tx_candata[0] = LED_green_efiCAN;
        CAN_send8(tx_candata);
    }

    EXTI->PR1 |= EXTI_PR1_PIF2;                    // reset pending register
}
...
void EXTI3_IRQHandler(void)
{
    ...
    if (!(GPIOB->IDR & MASK_S3))        // button S3 pressed
    {
        GPIOC->ODR ^= (1 << 7);         //LED3 toggle
        tx_candata[0] = LED_blue_efiCAN;
        CAN_send8(tx_candata);
    }

    EXTI->PR1 |= EXTI_PR1_PIF3;                    // reset pending register
}
