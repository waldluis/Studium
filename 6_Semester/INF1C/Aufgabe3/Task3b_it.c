 ...
 *
 *  Description:
 *      All interrupt service routines for Embedded I, Aufgabe 3
 *
 *
 *  Notes:
...
/* ------------------------------------------------------------------------------------ *\
 * method:  void TIM3_IRQHandler (void)
 *
 * ISR Name found in: startup_stm32l476xx.s
\* ------------------------------------------------------------------------------------ */
void TIM3_IRQHandler(void)
{
    ...
    else
    {
        ...
        {
            ticks = (0xffffffff - value1) + value2;
        }

        if(ticks != 0)
        {
            frequency = 1000000 / ticks;
        }
        first_captured = 0;
    }


    GPIOC->ODR ^= (0x1 << 7);     //LED3 test for timer output
    TIM3->SR &= ~(1<<4);
...
/* ------------------------------------------------------------------------------------ *\
 * method:  void TIM6_DAC_IRQHandler (void)
 *
 * Toggles LED3 (blue) on STefi board with 0.5 Hz frequency
 * ISR Name found in: startup_stm32l476xx.s
\* ------------------------------------------------------------------------------------ */
