...
void os_activate_task(int tid) {
 ...
 sys_log('A', tid);
  ...
  if(tq_len < TMAXQ)
  {
	  task_queue[tq_wpos] = tid; //Task in Liste schreiben
	  tq_wpos++;
	  tq_len++;	  
	  if(tq_wpos > TMAXQ) tq_wpos = 0; //Ringpuffer für schreiben
  }
 sys_enable_irq();
}
...
void os_schedule_next() {
 ...
 if (tq_len > 0)
 {
	 tid = task_queue[tq_rpos]; //Task aus Liste lesen
	 tq_rpos++;
	 tq_len--;
	 if (tq_rpos > TMAXQ) tq_rpos = 0; //Ringpuffer für lesen
 }
 sys_log('S', tid);
 sys_enable_irq(); 
 (*task_array[tid])();
